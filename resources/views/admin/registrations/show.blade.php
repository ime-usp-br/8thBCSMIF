            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="p-4 sm:p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="mb-8">
                        ...
                        <!-- AC2: Registration Date Display -->
                        <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 mb-6">
                            <div class="flex items-center justify-between">
                                <div>
                                    <h4 class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ __('Registration Date') }}</h4>
                                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">{{ __('When this registration was created') }}</p>
                                </div>
                                <div class="text-right">
                    ...
                    <!-- AC2: Payment Validation Panel -->
                    @if($registration->payments()->whereNotNull('payment_proof_path')->exists())
                    <div class="mb-8">
                        <div class="border-l-4 border-usp-blue-pri pl-4 mb-6">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                {{ __('Review and validate payment proof documents') }}
                            </p>
                        </div>

                        @php
                            $latestPaymentWithProof = $registration->payments()
                                ->whereNotNull('payment_proof_path')
                                ->latest()
                                ->first();
                        @endphp

                        @if($latestPaymentWithProof)
                        <!-- Payment Proof Details -->
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6">
                            <div>
                                <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('Payment Status') }}</p>
                                <p class="mt-1">
                                    @php
                                        $statusColors = [
                                            'pending' => 'bg-yellow-100 text-yellow-800',
                                    ...
                                    @endphp
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $statusColors[$latestPaymentWithProof->status] ?? 'bg-gray-100 text-gray-800' }}">
                                        {{ $statusLabels[$latestPaymentWithProof->status] ?? $latestPaymentWithProof->status }}
                                    </span>
                                </p>
                            </div>
                            <div>
                                <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('Payment Amount') }}</p>
                                <p class="mt-1 text-sm text-gray-900 dark:text-gray-100 font-medium">R$ {{ number_format($latestPaymentWithProof->amount, 2, ',', '.') }}</p>
                            </div>
                            @if($latestPaymentWithProof->payment_date)
                            <div>
                                <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('Payment Date') }}</p>
                                <p class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $latestPaymentWithProof->payment_date->format('d/m/Y H:i') }}</p>
                            </div>
                            @endif
                        </div>

                        @if($latestPaymentWithProof->notes)
                        <div class="mb-6">
                            <p class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">{{ __('Payment Notes') }}</p>
                            <div class="bg-gray-50 dark:bg-gray-700 rounded-md p-3 border border-gray-200 dark:border-gray-600">
                                <p class="text-sm text-gray-900 dark:text-gray-100 whitespace-pre-wrap">{{ $latestPaymentWithProof->notes }}</p>
                            </div>
                        </div>
                        @endif

                        <!-- Payment Proof Download -->
                        <div class="mb-6 bg-gray-50 dark:bg-gray-700 rounded-lg p-4 sm:p-6">
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                                <div class="mb-4 sm:mb-0">
                                    <h4 class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ __('Payment Proof Document') }}</h4>
                                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">{{ __('Download payment verification document') }}</p>
                                </div>
                                <a href="{{ route('admin.registrations.download-proof', $registration) }}" 
                                   class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-usp-blue-pri hover:bg-usp-blue-pri/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-usp-blue-pri transition-colors duration-200">
                        ...
                        @if($latestPaymentWithProof->status === 'pending_approval')
                        <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 sm:p-6">
                            <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                                <div class="flex-shrink-0">
                                    <h4 class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ __('Payment Validation Actions') }}</h4>
                                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">{{ __('Approve or reject this payment proof') }}</p>
                                </div>
                                <div x-data="{ 
                                    ...
                                    approve() {
                                        ...
                                        fetch(`{{ route('admin.registrations.approve-payment', $registration) }}`, {
                                            method: 'PATCH',
                                            headers: {
                                    ...
                                    reject() {
                                        ...
                                        this.isSubmitting = true;
                                        fetch(`{{ route('admin.registrations.reject-payment', $registration) }}`, {
                                            method: 'PATCH',
                                            headers: {
                                                'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content,
                                                'Content-Type': 'application/json',
                                    ...
                                    <div class="flex flex-wrap gap-3">
                                        <button @click="approve()" 
                                            ...
                                            <span x-show="!isSubmitting">{{ __('Approve Payment') }}</span>
                                        ...
                                        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                                            ...
                                            <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                                                <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                                    <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">{{ __('Reject Payment Proof') }}</h3>
                                                    <div class="mt-4">
                                                ...
                                                <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                                                    <button @click="reject()" 
                                                        ...
                                                        <span x-show="!isSubmitting">{{ __('Reject Payment') }}</span>
                                                        <span x-show="isSubmitting">{{ __('Processing...') }}</span>
                                                    </button>
