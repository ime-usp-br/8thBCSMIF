...
class FixOrphanedPaymentsCommandTest extends TestCase
{
    ...
    public function it_logs_error_on_payment_creation_failure(): void
    {
        ...
        // Create an event and a fee for it
        $eventWithFee = \App\Models\Event::factory()->create();
        $eventWithFee->fees()->create([
            'price' => 100.00,
            'participant_category' => 'professor_non_abe_professional', // Example category
            'participation_format' => 'in_person',
        ...
        // Mock the payments relationship to return a mock of the HasMany relationship builder
        $paymentsRelationMock = \Mockery::mock(\Illuminate\Database\Eloquent\Relations\HasMany::class);
        $paymentsRelationMock->shouldReceive('create')
            ->once()
            ->andThrow(new \Exception('Simulated payment creation error.'));

        // Replace the actual payments relationship with our mock
        $registration->shouldReceive('payments')
            ->once()
            ->andReturn($paymentsRelationMock);

        // Expect log calls
        \Illuminate\Support\Facades\Log::shouldReceive('info')
            ->once()
            ->with('Starting to fix orphaned payments...');

        \Illuminate\Support\Facades\Log::shouldReceive('error')
            ->once()
            ->with('Failed to fix orphaned payment for registration.', \Mockery::any());

        \Illuminate\Support\Facades\Log::shouldReceive('info')
            ->once()
            ->with('Finished. Fixed 0 orphaned payments.');

        // Run the Artisan command
        \Illuminate\Support\Facades\Artisan::call('registrations:fix-orphaned-payments');

        // Assert command output
        $output = \Illuminate\Support\Facades\Artisan::output();
        $this->assertStringContainsString("Failed to fix payment for registration ID: {$registration->id}. Error: Simulated payment creation error.", $output);
        $this->assertStringContainsString('Finished. Fixed 0 orphaned payments.', $output);
